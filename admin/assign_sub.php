    ...
    <div class="row">
        <div class="card card-plain">
            <div class="card-header">
                <h3 class="card-title font-weight-bold">Subject List</h3>
                    <p class="category">Sudject and their courses</p>
                        <?php
                            if(isset($_GET["mes1"])) {
                    ...
                    <table class="table tablesorter" id="">
                        ...
                        <tbody>
                            <?php
                                ...
                                INNER JOIN sub_course ON subjects.subject_id = sub_course.subject_id
                                            ...
                                            echo" 
                                                <tr>
                                                    ...
                                                    <td class='text-center'>
                                                        <a href='../sub_course_delete.php?sub_course_id={$r["sub_course_id"]}' onclick='javascript:confirmationDelete($(this));return false;'  class='btn btn-danger'>Delete</a>
                                                    </td>
                                                </tr>  "; 
                                    }
                            ?>
                        ...
                        <?php
                            for($page = 1; $page <= $number_of_pages; $page++){
                                    echo'
                                        <a href="assign_sub.php?page=' . $page . '" class="btn btn-primary" >'. $page .'</a>';
                            }
                        ?>
                </div>
            </div>
            <div class="card-body">
                <div class="col-lg-8">
                    <div class="col-lg-12 col-xl-12 offset-lg-3 offset-xl-3">
                        <div class="card">
                            <div class="card-header">
                                <h3 class="title">Subjects to Course</h3>
                                <p class="category">Assign Subjects to Course</p>
                            </div>
                            <div class="card-body">
                            <?php
                                    if(isset($_POST["submit2"])) { 
                                        ...
                                        $checkbox1 = $_POST['chkl'] ;
                                            for ($i=0; $i<sizeof ($checkbox1);$i++) {
                                                $dup=$db->query("SELECT * FROM sub_course WHERE course_id='$id' AND subject_id='".$checkbox1[$i]."'");
                                                if(mysqli_num_rows($dup)>0){
                                                    echo "
                                                        <script>window.open('assign_sub.php?mes2=Subject Already Assigned','_self') </script>";
                                                }
                                                else{
                                                    $sq="INSERT INTO sub_course(course_id,subject_id) VALUES ('{$_POST["course_id"]}','".$checkbox1[$i]."')";  
                                                    $r=$db->query($sq); 
                                                    if($r){
                                                        echo "
                                                            <script>window.open('assign_sub.php?mes1=Subject Assign Successful','_self') </script>"; 
                                                    }
                                                    else {
                                                        echo "
                                                            <script>window.open('assign_sub.php?mes2=Subject Assign Invalid','_self') </script>"; 
                                                    }
                                                }
                                            }
                                    }
